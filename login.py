...
class Example(Screen):
    ...
    #    TopBar = MDTopAppBar(title = "QRScanner", font_name= "Poppins/Poppins-SemiBold.ttf", font_size= "26sp", pos_hint= {"center_x": .5, "center_y": .1}, text_color = (0.49, 0.498, 0.498, 1), left_action_items = ([["arrow-left-circle-outline", lambda x : app.root.current = "bottom"]]))
       ...
       #ViewReport = MDFlatButton(text = "View Report", on_press = self.MovetoFourScreen, pos_hint = {'center_x': 0.5, 'center_y': 0.2})
        ...
        mycursor5.execute("SELECT stud_roll FROM canteen_table ")
        myresult5 = mycursor1.fetchall()
        
        mycursor6.execute("SELECT canteen_name FROM canteen_table ")
...
class Request(Screen):
    def __init__(self, **kwargs):
        ...
        my1.execute("SELECT RequestId FROM request_table WHERE status = 'Pending'")
        myr1 = my1.fetchall()
        
        my2.execute("SELECT user_name FROM request_table WHERE status = 'Pending'")
        myr2 = my1.fetchall()
        
        my3.execute("SELECT canteen_name FROM request_table WHERE status = 'Pending'")
        myr3 = my1.fetchall()
        
        my4.execute("SELECT AppRights FROM request_table WHERE status = 'Pending'")
        myr4 = my1.fetchall()
    ...
    def Insert_User(self):
        ...
        for i in range(0, len(User_id)):
            query = "SELECT * FROM request_table WHERE RequestId = {}".format(User_id[i])
            userName = ''
            userPassw = ''
            canteen_name = ''
            name = ''
            ...
            for user in users:
                ...
                InsertQuery = "INSERT INTO login_table (user_name, passw, canteen_name, name, AppRights) VALUES ('"+userName+"', '"+userPassw+"', '"+canteen_name+"', '"+name+"', '"+AppRights+"')"
                cursor2.execute(InsertQuery)
                Status = "UPDATE request_table SET status = 'Accepted' WHERE RequestId = " + User_id[i]
    ...
    def Accept_User(self):
        ...
        cursor1 = data.cursor()
        for i in range(0,len(User_id)):
            query = "UPDATE request_table SET status = 'Accepted' WHERE RequestId={}".format(User_id[i])
            cursor1.execute(query) 
            data.commit()
    ...
    def Reject_User(self):
        ...
        cursor1 = data.cursor()
        for i in range(0,len(User_id)):
            query = "UPDATE request_table SET status = 'Rejected' WHERE RequestId={}".format(User_id[i])
            cursor1.execute(query) 
            data.commit()
...
class Canteen_user(Screen):
    def __init__(self, **kwargs):
       ...
       #dropdown for delete in canteen
        ...
        my1.execute("SELECT id FROM login_table ")
        myr1 = my1.fetchall()
        
        my2.execute("SELECT name FROM login_table ")
        myr2 = my1.fetchall()
        
        my3.execute("SELECT canteen_name FROM login_table ")
        myr3 = my1.fetchall()
        
        my4.execute("SELECT user_name FROM login_table ")
        myr4 = my1.fetchall()
    ...
    def delete_canteen_check(self):
        ...
        for i in range(0,len(delete_Canteen_id)):
            query = "DELETE FROM login_table WHERE id={}".format(delete_Canteen_id[i])
            cursor1.execute(query) 
            data.commit()   
...
class LoginPage(MDApp):
    ...
    def set_snackLunch(self, category, stud_roll, stud_name, canteen_name):
        ...
        try:
            ...
            if val == date_string:
                ...
                self.cursor.execute(f"INSERT INTO canteen_table (entryDateTime, food_category, stud_roll, stud_name, canteen_name) VALUES( '{date_string}', ' {category.strip()}', '{stud_roll}', '{stud_name}', '{canteen_name}')")     
    ...
    def manageuser(self,obj):
        database1 = mysql.connector.Connect(host="localhost", user="root", password="@Harishvenkat2000", database= "silverpos")
        mycursor1 = database1.cursor()
        sql = 'INSERT INTO login_table(name,canteen_name,user_name,passw,AppRights) VALUES(%s,%s,%s,%s,%s)'
    ...
    def UpdateCanteen(self,obj):
        ...
        mycursor11 = database11.cursor()
        sql11 = 'UPDATE login_table SET name = %s,canteen_name = %s,user_name = %s, passw = %s, AppRights = %s WHERE canteen_name=%s'   
        kk = screen_manager.get_screen("Admin").ids.name4.text
        kk1 = screen_manager.get_screen("Admin").ids.Canteen_Name2.text
        kk2 = screen_manager.get_screen("Admin").ids.username.text
        kk3 = screen_manager.get_screen("Admin").ids.password4.text
        kk4 = screen_manager.get_screen("Admin").ids.apprights.text
        values11 =[kk,kk1,kk2,kk3,kk4]
        mycursor11.execute(sql11,values11)
        self.Cupdate1.dismiss()
        database11.commit() 
    ...
    def send_data(self, name, canteen_name, user_name, passw, apprights):
       ...
       try:
               self.cursor.execute(f"INSERT INTO request_table (name, canteen_name, user_name, passw, AppRights, status) VALUES('{name.text}', '{canteen_name.text}', '{user_name.text}', '{passw.text}', '{apprights.text}', 'Pending')")     
               self.database.commit() #this is importnt to insert query to database
               name.text = ""
               canteen_name.text = ""
               user_name.text = ""
    ...
    def on_save1(self, instance1, value1, date_range1):
        ...
        screen_manager.get_screen("Admin").ids.r_screen.add_widget(self.text_field1)
    ...
    def genrate_report(self):
        ...
        df.to_excel("Brief_report{}.xlsx".format(pd.datetime.today().strftime('%y%m%d')))
            ...
            for j in self.cursor:
                if password.text == j[0]:
                    ...
                    if AppRights == 'User':
                        ...
                        self.root.get_screen('bottom').ids.username.text = name.text
                        self.root.get_screen("bottom").ids.password.text = password.text
                    else:
                        self.root.current = "Admin"
                else:
    ...
    def Detailed_report(self):
        ...
        mycursor.execute("SELECT stud_name FROM canteen_table ")
        
        mycursor2.execute("SELECT stud_roll FROM canteen_table ")
        ...
        for y in det_date:
            for z in det_date1:
                if z in y:
                    ...
                    det_name.append(myresult2)
                    det_roll.append(myresult3)
                    det_food.append(rr)
                    det_t.append(z)
        ...
        dd = list(zip(det_t,det_name,det_roll,det_food,det_price))           
        df = pd.DataFrame(dd,columns=['Date','Name','RollNo','Food','Cost'])
        df.to_excel("Detailed_report{}.xlsx".format(pd.datetime.today().strftime('%y%m%d')))
    ...
    def Attendance_report(self):
        ...
        df.to_excel("Attendence_report{}.xlsx".format(pd.datetime.today().strftime('%y%m%d')))   
...
class Db_Operations:
    ...
    def Get_PendingRequests(self):
        sql = "SELECT user_name FROM request_table WHERE status = 'Pending'"
        self.mycursor.execute(sql)
